# -*- coding: utf-8 -*-
#------------------------------------------------------------
# TV Ultra 7K Updater v0.2 (21.10.2014)
...
def check_update(params):
    ...
    if i == 0:
        datamovie["Plot"]='¡Felicidades! Tu copia de [B]TV Ultra 7K[/B] tiene todos los módulos actualizados ;)[CR][CR][COLOR red]NOTA:[/COLOR] Bugs o sugerencias a [B]juarrox@gmail.com[/B]'
...
def update_file(params):
    plugintools.log("[tv.ultra.7k-0.3.0].Update_now "+repr(params))

    nobackup = 0
    title = params.get("title").replace("[COLOR white]", "").split("[")[0]
    runUpdate = xbmcgui.Dialog().yesno('TV Ultra 7K', '¿Desea actualizar el módulo [B]'+title+'[/B]?')
    
    if(runUpdate):
        local_file = params.get("extra")
        local_filename = local_file.split("/")[-1]
        remote_file = params.get("url")
        runBackup = xbmcgui.Dialog().yesno('TV Ultra 7K', '¿Desea crear una copia de seguridad de [B]'+title+'[/B]?')
        if(runBackup):
    ...
    if(runUpdate):
        if nobackup == -1:
            runUpdateWithBackup = xbmcgui.Dialog().yesno('TV Ultra 7K', 'Error al crear [B]'+backup+'[/B]', '¿Desea continuar con la actualización?')
        if nobackup != -1 or runUpdateWithBackup:            
            ...
            try:
                ...
                xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Actualización completada!", 3 , art+'icon.png'))
            except:
                pass
        else:
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Actualización cancelada!", 3 , art+'icon.png'))

    xbmc.executebuiltin("Container.Refresh")


def update_palco(params):
    plugintools.log("[tv.ultra.7k-0.3.0].update_palco "+repr(params))
    xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Espere...", 3 , art+'icon.png'))
    ...
    '''
        ...
        pass
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Error en la descarga", 3 , art+'icon.png'))
    '''
    ...
    '''except IOError:
        pass
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Actualización fallida", 3 , art+'icon.png'))
        '''
...
def parser_title(title):
    plugintools.log("[tv.ultra.7k-0.3.0].parser_title " + title)
