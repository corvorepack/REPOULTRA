...
addon_id = 'plugin.video.tv.ultra.7k'
...
def login_lolabits(defora=False):
      ...
      if re.search('003eA senha indicada n',logintest):
            ...
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "Contraseña incorrecta", 3 , art_lola+'icon.png'))            
            entrarnovamente(1)
            return False
            ...
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "Nombre de usuario no válido", 3 , art_lola+'icon.png'))                        
            entrarnovamente(1)
            return False
      elif re.search(username_lb,logintest):
            #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Login correcto! Cookies guardadas", 3 , art_lola+'icon.png'))            
            net.save_cookies(cookies)
            return True
      
      elif re.search('Erro',logintest) or link=='Erro':
            #opcao= xbmcgui.Dialog().yesno('Lolabits @ TV Ultra 7K', 'Sin conexión a Internet', "", "",'Reintentar', 'OK')
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "Sin conexión a Internet", 3 , art_lola+'icon.png'))            
            return False
      else: return False
...
def menu_principal(params):
    ...
    if login_lolabits():
        if status_lolabits == 'true': #addDir('Mi Lolabits',MainURL + username_lb,3, art_lola + 'pasta.png',2,True)
            datamovie["Plot"]='Acceder a mi cuenta en Lolabits. Asegúrate de haber configurado correctamente los datos de tu cuenta en la configuración de [B]TV Ultra 7K[/B]'
            ...
            datamovie["Plot"]='Configura tu cuenta Lolabits de [B]TV Ultra 7K[/B]'
...
def proxpesquisa_ab():
    ...
    if save_cookies:
        pass
        #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "Cookies guardadas", 3 , art_lola+'icon.png'))
    else:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "No se han guardado cookies", 3 , art_lola+'icon.png')) 
    ref_data = {'Accept':'*/*','Content-Type':'application/x-www-form-urlencoded','Host':'lolabits.es','Origin':'http://lolabits.es','Referer':url,'User-Agent':user_agent,'X-Requested-With':'XMLHttpRequest'}
...
def pastas(params):
    ...
    if re.search('ProtectedFolderChomikLogin',conteudo):
        ...
        if re.search('IsSuccess":false',teste):
              ...
              xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "Contraseña correcta", 3 , art_lola+'icon.png'))
              pastas_ref(url)
...
def analyzer(params):
    ...
    if re.search('ProtectedFolderChomikLogin',conteudo):
        #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Error en contraseña de carpeta ", 2 , art_lola+'icon.png'))    
...
def abrir_url_cookie(url,erro=True):
      ...
      if net.set_cookies(cookies):
          #xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Lolabits @ TV Ultra 7K', "Cargando cookies...", 3 , art_lola+'icon.png'))
          pass
      try:
          if status_lolabits == 'true':
              ref_data = {'Host': 'lolabits.es', 'Connection': 'keep-alive', 'Referer': 'http://lolabits.es/','Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8','User-Agent':user_agent,'Referer': 'http://abelhas.pt/'}
...
def versao_disponivel():
      try:
            ...
            ok = mensagemok('Lolabits.es','TV Ultra 7K no consigue conectar con el servidor','de actualización. Verifique su instalación','')
...
def lb_regex(params):
    ...
    if login_lolabits():
        ...
        if re.search('Pode acontecer que a mensagem de confirma',conteudo):
            mensagemprogresso.close()
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Por favor, revisa la configuración de Lolabits", 2 , art_lola+'icon.png'))
            return
    else:        
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('TV Ultra 7K', "Error al iniciar sesión en Lolabits ", 2 , art_lola+'icon.png'))
        sys.exit(0)
...
def loladownload(params):
    ...
    if os.path.exists(downloadPath):
        mypath=os.path.join(downloadPath,vidname)
        if os.path.isfile(mypath) is True:
            ok = mensagemok('TV Ultra 7K','Este archivo ya existe en el disco','','')
            return False
        else:              
            try:
                  dp = xbmcgui.DialogProgress()
