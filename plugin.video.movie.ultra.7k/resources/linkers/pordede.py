...
def pordede_linker0(params):
    plugintools.log("[%s %s] Pordede %s" % (addonName, addonVersion, repr(params)))
    ...
    if plugintools.get_setting("pordede_user") == "":
        ...
        plugintools.add_item(action="",url="",title="[COLOR lightblue][B] Pordede"+version+"[/B][COLOR lightblue]"+sc4+"[I] [/I]"+ec4,thumbnail=thumbnail,fanart=fanart,folder=False,isPlayable=False)
...
def pordede_link(params):
    ...
    try:
        ...
        if location:print '$'*25+'- Movies Ultra -'+'$'*25,location,'$'*69
