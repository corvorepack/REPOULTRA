...
import xbmc,xbmcgui,xbmcplugin,urllib2,os,sys,subprocess,xbmcvfs,socket,re,requests,shutil,plugintools
...
LOCAL_PORT = plugintools.get_setting('local_port')
VIDEO_PORT = plugintools.get_setting('video_port')
BUFER_SIZE = int(plugintools.get_setting('buffer_size'))
if(plugintools.get_setting('auto_ip')=='true'):
    LOCAL_IP=xbmc.getIPAddress()
else: LOCAL_IP=plugintools.get_setting('localhost')
...
def sopstreams(name,iconimage,sop):
	...
	labelname=name
	if plugintools.get_setting('addon_history') == "true":
	    try: add_to_history(labelname, str(sop),2, iconimage)
	    except: pass
	if not xbmc.getCondVisibility('system.platform.windows'):
	    if xbmc.getCondVisibility('System.Platform.Android'):
	    	if  plugintools.get_setting('external-sopcast') == "1":
            ...
            if config == False: mensagemok(translate(30000),translate(30027),translate(30028), translate(30029))
                ...
                res = handle_wait_socket(int(plugintools.get_setting('socket_time')),translate(30000),translate(30030))
                ...
                if res == True:
                        ...
                        handle_wait(int(plugintools.get_setting('stream_time')),translate(30000),translate(30031),segunda='')
                        try:
...
def sopstreams_builtin(name,iconimage,sop):
	try:
		global spsc
			...
			if os.uname()[4] == "armv6l" or os.uname()[4] == "armv7l" or plugintools.get_setting('openelecx86_64') == "true":
				if plugintools.get_setting('jynxbox_arm7') == "true":
			...
			elif plugintools.get_setting('openeleci386') == "true":
			...
			cmd = [str(plugintools.get_setting('android_sopclient')), str(sop), str(LOCAL_PORT), str(VIDEO_PORT)]
		...
		if plugintools.get_setting('debug_mode') == "false":
		...
		xbmc.sleep(int(plugintools.get_setting('wait_time')))
		res=False
		#counter=50
		counter = int(int(plugintools.get_setting("loading_time"))*2+10)
		...
		while counter > 0 and spsc.pid:
			...
			xbmc.sleep(400)
			counter -= 1
			mensagemprogresso.update(int((1-(counter/50.0))*100),"SopCast",translate(30035) % str(int(int(plugintools.get_setting("loading_time")) * (1-((1-(counter/50.0)))))))
			try:
				urllib2.urlopen(url)
				counter=0
		...
		if res:
			...
			if not xbmc.getCondVisibility('System.Platform.OSX'):
				...
				handle_wait(int(plugintools.get_setting('stream_time_osx')),translate(30000),translate(30031),segunda='')
	...
	if plugintools.get_setting('debug_mode') == "true":
		try:	
...
class SopWindowsPlayer(xbmc.Player):
      def __init__(self):
            ...
            if plugintools.get_setting('force_dvplayer') == 'true': xbmc.Player(xbmc.PLAYER_CORE_DVDPLAYER)
...
class streamplayer(xbmc.Player):
    ...
    def onPlayBackEnded(self):
        url = "http://"+LOCAL_IP+":"+str(VIDEO_PORT)+"/"
        xbmc.sleep(300)
        if os.path.exists("/proc/"+str(self.spsc_pid)) and xbmc.getCondVisibility("Window.IsActive(epg.xml)") and plugintools.get_setting('safe_stop')=="true":
