...
import xbmc,xbmcgui,xbmcplugin,urllib,xbmcvfs,os,subprocess,plugintools
...
def acestreams(name,iconimage,chid):
	...
	if plugintools.get_setting('engine_app') != '2' and plugintools.get_setting('engine_app') != '3':
		if plugintools.get_setting('aceplay_type') == "2":
			...
			pDialog.update(100)
			pDialog.close()
			ip_adress = plugintools.get_setting('ip_addr')
			proxy_port = plugintools.get_setting('aceporta')
...
def acestreams_builtin(name,iconimage,chid):
    if xbmc.getCondVisibility('system.platform.windows'):
        ...
        if plugintools.get_setting('shutdown-engine') == "true":
