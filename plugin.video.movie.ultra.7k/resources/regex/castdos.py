# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movie Ultra 7K Regex de cast247
...
def castdos(params):
    plugintools.log("[movie.ultra.7k-0.3.0].cast247 "+repr(params))
    url_user = {}
    ...
    if url_user.get("pageurl").find("domain=") <= 0:
        ...
        if body.find("Private Channel!") > 0 :
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Movie Ultra 7K', "Canal privado", 3 , art+'icon.png'))
            return 0
        else:
