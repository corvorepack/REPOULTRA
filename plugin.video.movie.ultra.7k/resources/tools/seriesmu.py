# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movie Ultra 7K Parser de SeriesMu
...
def seriesmu(params):
    plugintools.log("[movie.ultra.7k 0.3.0].SeriesMu")
...
def lista_letra(params):
    plugintools.log("[movie.ultra.7k 0.3.0].SeriesYonkis.Lista_letra")
...
def enlacesmu(params):
    ...
    for entry in capis:
        ...
        if host == "nowvideosx":
            ...
            url_link = getotherhost(url_link)
            if url.find("veehd") >= 0:
                server = "VeeHD"
                if lang_sub != "":
...
def gethttp_referer_headers(url,referer):
    plugintools.log("movie.ultra.7k-0.3.0.gethttp_referer_headers ")
