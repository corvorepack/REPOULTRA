# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Sports On Demand parser para Movie Ultra 7K
...
def sod0(params):
    plugintools.log("[movie.ultra.7k 0.3.3].Sports on Demand "+repr(params))
...
def davka0(params):
    plugintools.log("[movie.ultra.7k 0.3.3].Davka BT "+repr(params))
...
def sv_hockey0(params):
    plugintools.log("[movie.ultra.7k 0.3.3].Sport-video Hockey "+repr(params))
    ...
    for entry in matches:
        ...
        if url != "":
            ...
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Movie Ultra 7K', "URL no encontrada...", 3 , art+'icon.png'))
