# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Mundoplus.tv parser para Movie Ultra 7K
...
from resources.tools.media_analyzer import parser_title
...
def mundoplus_guide(params):
    ...
    title = title.replace("[COLOR white]", "").replace("[COLOR lightyellow]", "").replace("[I]", "").replace("[/I]", "").replace("[/COLOR]", "").replace("[Multiparser]", "").strip()
...
def mundoplus0(params):
    ...
    try:
        ...
        params["plot"]=texto
        texto = texto.lower()        
        if texto == "": errormsg = plugintools.message("Movie Ultra 7K","Por favor, introduzca serie a buscar");return errormsg
        else:
...
def mundoplus1(params):
    plugintools.log("[movie.ultra.7k 0.3.3].Mundoplus.tv búsqueda "+repr(params))
...
def gethttp_referer_headers(url,referer):
    plugintools.log("movie.ultra.7k 0.3.3 Gethttp_referer_headers "+url)
    request_headers=[]
