# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movie Ultra 7K Parser de Vipracing.org
...
def vip0(params):
    ...
    plugintools.add_item(action="", title= '[COLOR blue][B]Vipracing[/B][/COLOR]', url = "", thumbnail = thumbnail , fanart = fanart, folder = False, isPlayable = False) 
    plugintools.add_item(action="vipr_channels", title= '[COLOR white]Mostrar canales... [/COLOR]',url=params['url'], thumbnail = thumbnail , fanart = fanart, folder = True, isPlayable = False)
...
def server_vipracing(url_vipracing):
    #plugintools.log("[movie.ultra.7k-0.3.0].server_rtmp " + url_vipracing)
...
def canal_vipracing(canal):
    #plugintools.log("[movie.ultra.7k-0.3.0].canal_vipracing "+canal);#'http://vipracing.tv/index.php?/channel/opcion-2' ;#'http://vipracing.org/channel/opcion-1'
    if canal.find("1") >= 0:
        canal = "opcion-1"
    elif canal.find("2") >= 0:
        canal = "opcion-2"
...
def menu2_vipracing(params):
    #plugintools.log("[movie.ultra.7k-0.3.0].menu2_vipracing "+repr(params))
    ...
    for entry in matches:
        ...
        try:
            ...
            plugintools.add_item(action="launch_rtmp", title = "[COLOR white]Canal "+str(i)+'[/COLOR][COLOR green] [' + server + '][/COLOR]', url=url_vipracing, thumbnail = 'https://dl.dropbox.com/s/91wbjnwwwpubxjo/Vipracing.jpg' , fanart = 'https://dl.dropbox.com/sh/i4ccoqhgk7k1t2v/AAChTJOeg7LsgPDxxos5NSyva/fondo tv.jpg' , folder = False, isPlayable=True)
            i = i + 1
            if url == "":
...
def vipr_schedule(params):
    ...
    for entry in date:
        ...
        #print entry[1]
        date = entry[1]
        plugintools.add_item(action="", title = '[COLOR red][B]'+date+'[/B][/COLOR]' , url="" , thumbnail = 'https://dl.dropbox.com/s/91wbjnwwwpubxjo/Vipracing.jpg' , fanart = 'https://dl.dropbox.com/sh/i4ccoqhgk7k1t2v/AAChTJOeg7LsgPDxxos5NSyva/fondo tv.jpg', folder = False, isPlayable = False )
            ...
            if item_evento.find(":") >=0:
                ...
                while j < len(evento):
                    ...
                    if item_evento in canales_dict :
                        ...
                        if anterior != evento_final:
                            ...
                            if title_fixed != anterior:
                                if title_fixed.startswith("[COLOR lightyellow] [/COLOR]") == False:
                                    title_fixed = '[COLOR lightyellow]'+anterior+' [/COLOR][COLOR lightgreen][I][VipRacing][/I][/COLOR]'
                                    hora_title = title_fixed.replace("[COLOR lightyellow]", "")[0:6].strip();plugintools.log("hora_title= "+hora_title)
                                    title_fixed = title_fixed.replace(hora_title, '[COLOR orange][B][I]'+hora_title+'[/I][/B][/COLOR] ')
                                    plugintools.log("title_fixed2= "+title_fixed)
                                    time_event=evento_final[0:5]
                                    evento_final=evento_final.replace(time_event, "").strip()
                                    title = '[COLOR blue][B]'+time_now+'[COLOR red] '+time_event+'[COLOR white] '+evento_final+' ('+canal+') [COLOR green][VipRacing][/B][/COLOR]'
                                    plugintools.add_item(action="vipracing2", title = title , url=url , thumbnail = "" , info_labels = datamovie , plot = lista_canales , fanart = "", folder = False, isPlayable = False )
                ...
                while j < len(evento):
                    ...
                    if item_evento in canales_dict :
                        ...
                        if url:
                            if evento_final[2]==".":
                                plugintools.log("evento_final= "+evento_final)
                                time_event=evento_final[0:5].strip()
                                evento_final=evento_final.replace(time_event, "")
                                title = '[COLOR blue][B]'+time_now+'[COLOR red] '+time_event+'[COLOR white] '+evento_final+' ('+canal+') [COLOR green][VipRacing][/B][/COLOR]'
                                plugintools.add_item(action="vipracing2", title = title , plot=canal , url=url , thumbnail = "" , fanart = "", folder = False, isPlayable = True )
                                ultrasp.write("Title:"+title+"\nAction:"+params.get("action")+"\nURL:"+url+"\n\n")
                                j = j + 1
                                continue
                            else:
